...
function asXlm(value) {
    return `${Math.ceil(value / 25)} XLM`;
}
function toXlmAmount(amountUSD) {
...
function mapHorizonError(error, fallback = "Stellar transaction failed.") {
    const codes = error?.response?.data?.extras?.result_codes;
    const txCode = codes?.transaction;
    const opCode = Array.isArray(codes?.operations) ? codes.operations[0] : undefined;
    if (error?.response?.status === 404)
        return "Stellar account not found on testnet.";
    if (txCode === "tx_bad_seq")
        return "Bad sequence. Refresh the page and try again.";
    if (opCode === "op_underfunded" || txCode === "tx_insufficient_balance")
        return "Wallet has insufficient XLM for this payment and network fee.";
    if (opCode === "op_no_destination")
        return "Funding destination account does not exist on Stellar testnet.";
    if (opCode === "op_malformed")
        return "Stellar payment operation is malformed.";
    if (opCode === "op_no_trust")
        return "Destination is missing the required trustline.";
    return error?.response?.data?.detail || error?.message || fallback;
}
function resolveFundingDestination(invoice, destinationWallet) {
    const candidates = [
        destinationWallet,
...
export function fundInvoice(id, funder) {
    const store = getStore();
...
export async function buildFundInvoiceTransaction(input) {
    ...
    try {
        ...
        const nativeBalance = Number(sourceAccount.balances.find((balance) => balance.asset_type === "native")?.balance || 0);
...
export function repayInvoice(id) {
    const store = getStore();
    const invoice = store.invoices.find((item) => item.id === id);
    if (!invoice)
        throw new Error("Invoice not found.");
    if (invoice.status !== "funded")
        throw new Error("Invoice must be funded before repayment.");
...
export async function parseInvoice(fileBase64, mimeType) {
...
function buildAssistantFallback(question, walletAddress, view) {
    const store = getStore();
