...
function requireMethod(req: Request, allowed: Method[]) {
...
export async function invoicesHandler(req: Request, res: Response) {
  try {
    ...
    res.status(201).json({ success: true, data: await service.createInvoice(auth, invoiceCreateSchema.parse(req.body) as Parameters<typeof service.createInvoice>[1]) });
  } catch (error) {
    sendError(res, error);
  }
}
...
export async function invoiceActionHandler(req: Request, res: Response) {
  try {
...
export async function parseInvoiceHandler(req: Request, res: Response) {
  try {
    requireMethod(req, ["POST"]);
    await assertRateLimit(req);
    await getServerlessAuthContext(req);
...
export async function chatHandler(req: Request, res: Response) {
  try {
    ...
    await assertRateLimit(req);
    await getServerlessAuthContext(req);
    res.status(200).json({ success: true, data: await service.askAssistant(chatSchema.parse(req.body) as Parameters<typeof service.askAssistant>[0]) });
  } catch (error) {
    sendError(res, error);
  }
}

export async function trackVisitorHandler(req: Request, res: Response) {
  try {
    requireMethod(req, ["POST"]);
    await assertRateLimit(req);
    const auth = await getServerlessAuthContext(req);
...
export async function invoiceAllHandler(req: Request, res: Response) {
  try {
    requireMethod(req, ["GET"]);
    await assertRateLimit(req);
    const auth = await getServerlessAuthContext(req);
...
export async function invoiceUploadHandler(req: Request, res: Response) {
  try {
    requireMethod(req, ["POST"]);
    await assertRateLimit(req);
    await getServerlessAuthContext(req);
...
export async function invoiceFundHandler(req: Request, res: Response) {
  try {
    requireMethod(req, ["POST"]);
    await assertRateLimit(req);
    await getServerlessAuthContext(req);
