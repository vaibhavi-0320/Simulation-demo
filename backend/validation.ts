...
const strippedTextOptions: sanitizeHtml.IOptions = {
  allowedTags: [],
  allowedAttributes: {},
...
const cleanString = (maxLength: number) =>
  z.string().transform(sanitizeString).pipe(z.string().min(1).max(maxLength));

const optionalCleanString = (maxLength: number) =>
  z.string().transform(sanitizeString).pipe(z.string().max(maxLength)).optional();
...
const PAGE_VALUES = ["dashboard", "simulation", "marketplace", "deal", "activity", "upload", "portfolio", "landing"] as const;
const STEP_VALUES = ["upload", "parsing", "review", "funding", "repayment"] as const;
