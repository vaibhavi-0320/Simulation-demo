 ...
 */
export async function startHorizonListener(contractAddress) {
    try {
 ...
 */
async function handleHorizonEvent(tx) {
    try {
 ...
 */
export function isHorizonSynced() {
    return horizonSynced;
}
/**
 * Stop the Horizon listener
 */
export function stopHorizonListener() {
    horizonSynced = false;
 ...
 */
export async function pollHorizonTransactions(contractAddress, pollInterval = 10000) {
    setInterval(async () => {
        try {
            const server = new Horizon.Server(HORIZON_URL);
