...
export async function connectManualWallet(provider, publicKey) {
    if (provider === "demo") {
        return {
...
function getLocalInvoices() {
    const stored = safeStorage.get("invoices", []);
    if (!stored.length) {
...
export async function fundInvoice(invoiceId, amountUSD, deal) {
...
export async function simulateOnTestnet(amountUSD, deal) {
    const connected = await isConnected();
    ...
    const expectedReturn = parseFloat((amountUSD * ((deal.apy || 0) / 100) * ((deal.durationDays || 180) / 365)).toFixed(2));
...
export async function fundInvoiceOnBackend(invoiceId, funder, provider, amountUSD) {
    const current = getLocalInvoices();
...
export async function repayInvoiceOnBackend(invoiceId) {
    const current = getLocalInvoices();
    const invoice = current.find((item) => item.id === invoiceId);
    if (!invoice) {
        throw new Error("Invoice not found");
...
export async function buildSorobanEscrowPreview(invoiceId, amount) {
